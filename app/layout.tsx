...
export default function RootLayout({
  ...
  return (
    <html lang="en">
      <body className="antialiased flex min-h-screen flex-col bg-brand-black text-brand-cream">
        <header className="border-b border-brand-brown bg-brand-black">
          <nav className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-6 py-4">
            ...
            <ul className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm font-medium text-brand-tan">
              {navLinks.slice(1).map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="transition hover:text-brand-gold-light"
                  >
                    {link.label}
                  </Link>
                </li>
        ...
        <footer className="border-t border-brand-brown bg-brand-black px-6 py-8 text-center text-sm text-brand-tan-dark">
          <p>© {new Date().getFullYear()} Black Sheep Services. Built by Jose Vilchis.</p>
          <p className="mt-2">
            <a href="tel:6308867419" className="hover:text-brand-gold-light">
              (630) 886-7419
            </a>
            {" · "}
            <a
